...
const NETWORK_STATUS = [
  { value: "oon", label: "Out-of-network with major commercial plans" },
  { value: "partial", label: "Mixed / some plans OON" },
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          network_status: fd.get("network_status") || undefined,
    ...
    return (
      <div className="bg-black/20 p-8 text-center">
        ...
        <p className="font-body text-white/60 text-sm font-light">
          Thank you. We will review your claim details and reach out shortly.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="cr-name" className={labelClass}>Your name <span className="text-kronos-cyan">*</span></label>
          <input
            type="text" id="cr-name" name="contact_name" required
        ...
        <div>
          ...
          <input
            type="text" id="cr-title" name="title"
      ...
      <div>
        <label htmlFor="cr-practice" className={labelClass}>Practice name <span className="text-kronos-cyan">*</span></label>
        <input
          type="text" id="cr-practice" name="practice_name" required
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="cr-phone" className={labelClass}>Phone <span className="text-kronos-cyan">*</span></label>
          <input
        ...
        <div>
          <label htmlFor="cr-email" className={labelClass}>Work email <span className="text-kronos-cyan">*</span></label>
          <input
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="cr-specialty" className={labelClass}>Specialty <span className="text-kronos-cyan">*</span></label>
          <div className="relative">
            <select
              id="cr-specialty" name="specialty" required
        ...
        <div>
          <label htmlFor="cr-state" className={labelClass}>State</label>
          <input
            type="text" id="cr-state" name="state"
      ...
      <div>
        <label className={labelClass}>Network status</label>
        <div className="space-y-2.5 mt-1">
          {NETWORK_STATUS.map((opt) => (
            <label key={opt.value} className="flex items-start gap-3 cursor-pointer group">
              <input
                type="radio" name="network_status" value={opt.value}
      ...
      <div>
        <label htmlFor="cr-volume" className={labelClass}>Monthly OON claims (approx.)</label>
        <div className="relative">
          <select id="cr-volume" name="monthly_oon_claims" className={selectClass} defaultValue="">
            <option value="">Select range</option>
            {MONTHLY_CLAIMS.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          <div className="pointer-events-none absolute inset-y-0 right-3 flex items-center">
            <svg className="w-4 h-4 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
          </div>
      ...
      <div>
        <label htmlFor="cr-message" className={labelClass}>Message (optional)</label>
        <textarea
      ...
      >
        {isSubmitting ? (
          ...
          <>Request Free Claim Review<ArrowRight className="w-3 h-3" aria-hidden="true" /></>
        )}
      </button>
    </form>
  );
